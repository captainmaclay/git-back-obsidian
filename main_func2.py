...
def sync_files_to_fake(target_dir: Path | None = None):
    ...
    log_both(f"[DEBUG-SYNC] ИТОГО: файлов просмотрено {total}, скопировано {copied}, пропущено {skipped}")
    ...
    log_both(f"Синхронизация завершена. Удалено файлов: {deleted_count}")
