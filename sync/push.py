...
from sync.observer import start_observer, stop_observer, is_observer_running, restart_observer

from sync.file_copier import sync_changed_files

from core.logger import log_main, log_both, log_soft, init_logger
...
script_dir = Path(__file__).resolve().parents[1]  # корень проекта
...
def github_api_main_commit_count(max_check: int = 2) -> int:
    """
    Возвращает число коммитов в ветке main (но не больше max_check).
    0    — репозиторий пустой (коммитов ещё нет).
    -1   — не удалось определить (сетевая/иная ошибка) → вызывающий код
           обязан трактовать это как «force запрещён».
    """
    headers = {"Authorization": f"token {GITHUB_TOKEN}", "Accept": "application/vnd.github.v3+json"}
    url = f"https://api.github.com/repos/{GITHUB_USERNAME}/{GITHUB_REPO}/commits"
    params = {"sha": "main", "per_page": max_check}

    for attempt in range(1, 4):
        try:
            r = requests.get(url, headers=headers, params=params, timeout=15)
            if r.status_code in (404, 409):
                # 409 — пустой репозиторий, 404 — ветки/репо нет → коммитов нет
                return 0
            r.raise_for_status()
            return len(r.json())
        except (requests.Timeout, requests.ConnectionError) as e:
            log_main(f"[FORCE-GUARD] Сетевая ошибка (попытка {attempt}): {e}")
            time.sleep(3 * attempt)
        except Exception as e:
            log_main(f"[FORCE-GUARD] Ошибка (попытка {attempt}): {e}")
            break
    return -1


def is_force_push_allowed() -> bool:
    """
    Force-push разрешён ТОЛЬКО если в main нет других пушей, кроме initial commit
    (то есть коммитов не больше одного). Во всех остальных случаях — запрещён,
    чтобы force не перезаписал существующую историю.
    Если число коммитов определить не удалось — тоже запрещаем (безопасное поведение).
    """
    count = github_api_main_commit_count()
    if count < 0:
        log_main("[FORCE-GUARD] Не удалось определить число коммитов в main → force ЗАПРЕЩЁН")
        return False
    allowed = count <= 1
    log_both(f"[FORCE-GUARD] Коммитов в main: {count} → force {'РАЗРЕШЁН' if allowed else 'ЗАПРЕЩЁН'}")
    return allowed
...
def github_api_force_push_from_tree(tree_sha, commit_message):
    ...
    allow_force = is_force_push_allowed()
    ...
    try:
        ...
        payload_ref = {"sha": new_commit_sha, "force": allow_force}
        r_ref = requests.patch(f"{base_url}/git/refs/heads/main", headers=headers, json=payload_ref, timeout=30)

        if r_ref.status_code == 422 and not allow_force:
            log_main("[FORCE-GUARD] Обновление main отклонено (не fast-forward) — "
                     "история защищена, force запрещён при наличии других пушей")
            return None
        r_ref.raise_for_status()

        log_both(f"[PUSH] выполнен (force={allow_force})")
